...
let hr = 0;
const k = W(), T = W(), qe = W(), f = W(), C = W(), H = W(), O = W();
function W() {
  return 2 ** ++hr;
}
        ...
        if (i.includeExplanation !== "scopeName")
          for (const G of n.settings) {
            ...
            switch (typeof G.scope) {
              case "string":
                z = G.scope.split(/,/).map((Ae) => Ae.trim());
                break;
              case "object":
                z = G.scope;
                break;
            ...
            lt.push({
              settings: G,
              selectors: z.map((Ae) => Ae.split(/ /))
            });
          }
        Ne.explanation = [];
        let ct = 0;
        for (; I + ct < $; ) {
          const G = y[_], z = h.substring(
            G.startIndex,
            G.endIndex
          );
          ct += z.length, Ne.explanation.push({
            content: z,
...
function bn(t, e) {
  ...
  if (e[t]) {
    ...
    for (; e[t]; ) {
      if (t = e[t], n.has(t))
        throw new q(`Circular alias \`${Array.from(n).join(" -> ")} -> ${t}\``);
      n.add(t);
    }
  }
  return t;
...
class ao extends mr {
  ...
  loadLanguages(e) {
    ...
    if (r.length) {
      ...
      throw new q(`Missing languages ${r.map(([o]) => `\`${o}\``).join(", ")}, required by ${i.map(([o]) => `\`${o}\``).join(", ")}`);
    }
    for (const [i, o] of n)
...
function _n(t) {
  ...
  function a(g) {
    ...
    if (!w)
      throw new q(`Language \`${g}\` not found, you may need to load it first`);
    return w;
  }
  function c(g) {
    if (g === "none")
    ...
    if (!w)
      throw new q(`Theme \`${g}\` not found, you may need to load it first`);
    return w;
  }
  function u(g) {
    y();
...
//# sourceMappingURL=bundle-full-D9WA66PB.js.map
